...
public class PDUDeviceServiceImpl implements PDUDeviceService {
    ...
    public PageResult<PDUDeviceDO> getPDUDevicePage(PDUDevicePageReqVO pageReqVO) {
        ...
        if (StringUtils.isNotEmpty(pageReqVO.getDevKey())){
            boolean matches = pageReqVO.getDevKey().matches("[\\u4e00-\\u9fa5]+");
            if (matches){
                BusinessAssert.error(50020,"网络地址出现中文，请输入正确的PDU的网络地址");
            }
        }
        if (Objects.equals(pageReqVO.getCurbance(),1)) {
...
//                pduDeviceDO.setCvol(new BigDecimal(volArr.size() > 2 ? volArr.get(2) : 0).setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue());
    ...
    public PageResult<PDULineRes> getPDULineDevicePage(PDUDevicePageReqVO pageReqVO) {
        try {
            ...
            String index = "pdu_hda_line_realtime";
//            if (pageReqVO.getTimeType() == 0 || pageReqVO.getOldTime().toLocalDate().equals(pageReqVO.getNewTime().toLocalDate())) {
...
//            }
    ...
    @Override
    public MaxCurAndOtherData getPDUMaxCurData(PDUDevicePageReqVO pageReqVO) {
        try {
...
//            }
            ...
            if (pageReqVO.getFlagVlaue() == 0) {
                // 排序字段
                searchSourceBuilder.sort("cur_value", SortOrder.DESC);
                // 限制字段，只取需要的字段
                includeFields = new String[]{"cur_value", "create_time", "line_id", "pdu_id"};
            } else {
                // 排序字段
                searchSourceBuilder.sort("pow_active", SortOrder.DESC);
                // 限制字段，只取需要的字段
                includeFields = new String[]{"pow_active", "create_time", "line_id", "pdu_id"};
            }
            ...
            if (hits.getHits().length > 0) {
                SearchHit hit = hits.getAt(0);// 由于已按 cur_max_value 降序排序，取第一个即为最大值
                Map<String, Object> sourceAsMap = hit.getSourceAsMap();
                // 构建结果对象
                if (pageReqVO.getFlagVlaue() == 0) {
                    BigDecimal value = BigDecimal.valueOf((Double) sourceAsMap.get("cur_value"));
                    result.setMaxValue(BigDemicalUtil.setScale1(value,2).toString());
                    result.setMaxTime(new DateTime(sourceAsMap.get("create_time").toString(), "yyyy-MM-dd HH:mm:ss"));
                    result.setLineId((Integer) sourceAsMap.get("line_id"));
                    result.setPduId((Integer) sourceAsMap.get("pdu_id"));
                    result.setUnit("A");
                } else {
                    // 构建结果对象
                    BigDecimal value = BigDecimal.valueOf((Double) sourceAsMap.get("pow_active"));
                ...
                PduIndex pduIndex = pDUDeviceMapper.selectOne(new LambdaQueryWrapper<PduIndex>().eq(PduIndex::getId, result.getPduId()).last("limit 1"));
                result.setDevKey(pduIndex.getPduKey());
                result.setLocation(getLocationByDevKey(result.getDevKey()));
    ...
    public Map getHistoryDataByDevKey(String devKey, String type) {
        ...
        switch (type) {
            ...
            case "oneMonth":
                ...
                startTime = formatter.format(now.minusMonths(1));
                indexLine = "pdu_hda_line_day";
                heads = new String[]{"pdu_id", "line_id", "power_factor_avg_value", "power_factor_max_value", "power_factor_max_time",
                        "power_factor_min_value", "power_factor_min_time", "create_time"};
                break;
            default:
        ...
        if (pduIndex1 != null) {
            ...
            for (int lineId = 1; lineId <= 3; lineId++) {
                ...
                if (lineData != null && !(((List<PduHdaLineHouResVO>) lineData.get("data")).isEmpty())) {
                    ...
                    if (dataType != 0) {
                        result.put("curMaxValue" + curResBase.getSeries().size(), currentResult.maxCurValue);
                        result.put("curMaxTime" + curResBase.getSeries().size(), sdf.format(currentResult.maxCurTime));
                        result.put("curMinValue" + curResBase.getSeries().size(), currentResult.minCurValue);
                        result.put("curMinTime" + curResBase.getSeries().size(), sdf.format(currentResult.minCurTime));
                        result.put("volMaxValue" + volResBase.getSeries().size(), voltageResult.maxVolValue);
                        result.put("volMaxTime" + volResBase.getSeries().size(), sdf.format(voltageResult.maxVolTime));
                        result.put("volMinValue" + volResBase.getSeries().size(), voltageResult.minVolValue);
                        result.put("volMinTime" + volResBase.getSeries().size(), sdf.format(voltageResult.minVolTime));
                    } else {
                        result.put("curMaxValue" + curResBase.getSeries().size(), currentResult.maxCurValue);
                        result.put("curMaxTime" + curResBase.getSeries().size(), "无");
                        result.put("curMinValue" + curResBase.getSeries().size(), currentResult.minCurValue);
                        result.put("curMinTime" + curResBase.getSeries().size(), "无");
                        result.put("volMaxValue" + volResBase.getSeries().size(), voltageResult.maxVolValue);
                        result.put("volMaxTime" + volResBase.getSeries().size(), "无");
                        result.put("volMinValue" + volResBase.getSeries().size(), voltageResult.minVolValue);
                        result.put("volMinTime" + volResBase.getSeries().size(), "无");
                    }

                    result.put("curName" + curResBase.getSeries().size(), "B路L" + lineId + "相电流");
                    result.put("volName" + volResBase.getSeries().size(), "B路L" + lineId + "相电压");
                }
...
//        curList.sort(Collections.reverseOrder());
    ...
    public List<PduTrendVO> getPudBalanceTrend(Integer pduId, Integer timeType) {
        ...
        try {
            ...
            switch (timeType) {
                ...
                case 2:
                    index = "pdu_hda_line_hour";
                    startTime = LocalDateTimeUtil.format(now.minusHours(72), "yyyy-MM-dd HH:mm:ss");
                    endTime = LocalDateTimeUtil.format(now, "yyyy-MM-dd HH:mm:ss");
                    break;
                case 3:
                    index = "pdu_hda_line_day";
                    startTime = LocalDateTimeUtil.format(now.minusMinutes(1), "yyyy-MM-dd HH:mm:ss");
                    endTime = LocalDateTimeUtil.format(now, "yyyy-MM-dd HH:mm:ss");
                    break;
                default:
            }
    ...
    @Override
    public BalancedDistributionStatisticsVO getBalancedDistribution(int curbance) {
        if (Objects.equals(curbance, 1)) {
            return pDUDeviceMapper.getBalancedDistribution();
        }
        return pDUDeviceMapper.getBalancedDistributionVol();
    }
        ...
        builder.aggregation(
                AggregationBuilders.terms("group_by_pdu_id")
                        ...
                        .subAggregation(AggregationBuilders.terms("by_line_id")
                                ...
                                .subAggregation(AggregationBuilders.max("max_cur").field("cur_value"))
                                .subAggregation(AggregationBuilders.topHits("top_docs")
                                        .size(1)
                                        .fetchSource(new String[]{"create_time"}, null)
                                        .sort(SortBuilders.fieldSort("cur_value").order(SortOrder.DESC))))
        );
        ...
        for (Terms.Bucket boxIdBucket : groupByBoxId.getBuckets()) {
            ...
            for (Terms.Bucket lineIdBucket : byLineId.getBuckets()) {
                ...
                if (topHits.getHits().getHits().length != 0) {
                    ...
                    maxValueAndCreateTime.setMaxTime(new DateTime(sourceAsMap.get("create_time").toString(), "yyyy-MM-dd HH:mm:ss"));
                }
        ...
        builder.aggregation(
                AggregationBuilders.terms("group_by_pdu_id")
                        ...
                        .subAggregation(AggregationBuilders.terms("by_line_id")
                                ...
                                .subAggregation(AggregationBuilders.max("max_pow").field("pow_active"))
                                .subAggregation(AggregationBuilders.topHits("top_docs")
                                        .size(1)
                                        .fetchSource(new String[]{"create_time"}, null)
                                        .sort(SortBuilders.fieldSort("pow_active").order(SortOrder.DESC))))
        );
