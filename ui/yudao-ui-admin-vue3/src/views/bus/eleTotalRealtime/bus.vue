...
const getList = async () => {
  ...
  try {
    ...
    const data = await EnergyConsumptionApi.getBusEleTotalRealtime(queryParams)
...
const handleExport = async () => {
  try {
    ...
    const data = await EnergyConsumptionApi.getBusEleTotalRealtimeExcel(queryParams, axiosConfig)
    await download.excel(data, '始端箱实时能耗.xlsx')
  } catch (error) {
    // 处理异常
