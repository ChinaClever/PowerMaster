<template>
  <CommonMenu :dataList="navList" @check="handleCheck" navTitle="插接箱环境数据" >
    <template #NavInfo>
      <br/>    <br/> 
        <div class="nav_data">
          ...
          <el-table-column :key="column.prop" :label="column.label" :align="column.align" :prop="column.prop" :formatter="column.formatter" :width="column.width" v-if="column.istrue" >
            <template #default="{ row }">
              <div v-if="column.slot === 'actions'">
                <el-button link type="primary" @click="toDetails(row.box_id, row.ip_addr, row.address)">详情</el-button>
              </div>
            </template>
          </el-table-column>
        </template>
...
const handleExport = async () => {
  try {
    ...
    const data = await EnvDataApi.exportBoxEnvHistoryData(queryParams, axiosConfig)
    await download.excel(data, '插接箱环境历史数据.xlsx')
  } catch (error) {
    // 处理异常
...
const getNavNewData = async() => {
  const res = await EnvDataApi.getBoxEnvNavNewData()
