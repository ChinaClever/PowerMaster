...
const getList = () => {
  ...
  try {
    ...
    cabinetEqData.value.eq = fakeData.map((item) => item.cabinetEq);
    cabinetEqData.value.name = fakeData.map((item) => item.cabinetName);
  }finally{
    loading.value = false
  }
}
...
const initChart = () => {
  ...
  if (cabinetRankChartContainer.value && instance) {
    ...
    cabinetChart.setOption({
      ...
      yAxis: { type: 'value'},
      series: [
        {name:"耗电量",  type: 'bar', data: cabinetEqData.value.eq, label: { show: true, position: 'top' }},// 你可以根据需要选择标签的位置，比如 'top', 'insideTop', 'inside', 等等
      ],
    });
...
// 监听类型颗粒度
//eqNew,powNew,temNew
watch([ () => queryParams.eqGranularity, () => queryParams.powGranularity, () => queryParams.temGranularity], (eqNew) => {
