      ...
      <div v-show="switchValue == 0  && list.length > 0" class="arrayContainer">
        <div class="arrayItem" v-for="item in list" :key="item.devKey">
          ...
          <div class="content">
            ...
            <div class="info">                  
              <div v-if="item.avol != null">A相电压：{{item.avol.toFixed(2)}}V</div>
              <div v-if="item.bvol != null" >B相电压：{{item.bvol.toFixed(2)}}V</div>
              <div v-if="item.cvol != null" >C相电压：{{item.cvol.toFixed(2)}}V</div>
              <!-- <div >网络地址：{{ item.devKey }}</div> -->
              <!-- <div>AB路占比：{{item.fzb}}</div> -->
            </div>
          </div>
