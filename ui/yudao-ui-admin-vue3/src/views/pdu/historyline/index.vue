...
function customTooltipFormatter(params: any[]) {
  ...
  params.forEach(function(item) {
    switch( item.seriesName ){
      case '平均有功功率(kW)':
        ...
        break;
      case '最大有功功率(kW)':
        tooltipContent += item.marker +' 发生时间: ' +activePowMaxTimeData.value[item.dataIndex] +  ' ' + item.seriesName + ': ' + item.value + '<br/>';
        break;
      case '最小有功功率(kW)':
        tooltipContent += item.marker +' 发生时间: ' +activePowMinTimeData.value[item.dataIndex] +  ' ' + item.seriesName + ': ' + item.value + '<br/>';
        break;
      case '平均视在功率(kVA)':
        tooltipContent += item.marker +' 记录时间: ' +params[0].name +  ' ' + item.seriesName + ': ' + item.value + '<br/>';
        break;
      case '最大视在功率(kVA)':
        tooltipContent += item.marker +' 发生时间: ' +apparentPowMaxTimeData.value[item.dataIndex] +  ' ' + item.seriesName + ': ' + item.value + '<br/>';
        break;
      case '最小视在功率(kVA)':
      ...
      case '最小电压(V)':
        ...
        break;
      case '功率因素': 
        tooltipContent += item.marker +' 记录时间: ' +params[0].name +  ' ' + item.seriesName + ': ' + item.value + '   记录时间: ' +params[0].name + '<br/>';
        break;
        case '有功功率(kW)':
      case '视在功率(kVA)':
      case '总有功功率(kW)':
  ...
  if(params[0].seriesName == "总有功功率(kW)"|| params[0].seriesName == "总视在功率(kVA)"||params[0].seriesName == "有功功率(kW)"|| params[0].seriesName == '视在功率(kVA)'||params[0].seriesName == "电流(A)"|| params[0].seriesName == '电压(V)'){
  ...
  realtimeChart?.on('legendselectchanged', function (params) {
    ...
    switch (legendName) {
     ...
     if (params.selected[legendName]){
      ...
      case '功率因素':
      if (params.selected[legendName]){
          optionsToUpdate = {  "功率因素": true , "总有功功率(kW)": false, "总视在功率(kVA)": false};
      }
        break;
...
function setupLegendListener1(realtimeChart) {
  realtimeChart?.on('legendselectchanged', function (params) {
    ...
    if (!legendData.includes('平均电压(V)')) {
      ...
      case '最小有功功率(kW)':
      if (params.selected[legendName]){
        optionsToUpdate = { "平均有功功率(kVA)": true, "最大有功功率(kVA)": true, "最小有功功率(kVA)": true,  "平均电流(A)": false, "最大电流(A)": false, "最小电流(A)": false,};
      }
        break;
    ...
    realtimeChart?.setOption({
      legend: {
        data: ['平均电流(A)', '最大电流(A)', '最小电流(A)','平均有功功率(kW)', '最大有功功率(kW)', '最小有功功率(kW)','平均视在功率(kVA)', '最大视在功率(kVA)', '最小视在功率(kVA)'],
        selected: optionsToUpdate
      },
    });
    ...
    switch (legendName) {
      ...
      case '最小电流(A)':
      if (params.selected[legendName]){
        optionsToUpdate = { "平均电流(A)": true, "最大电流(A)": true, "最小电流(A)": true, "平均电压(V)": false, "最大电压(V)": false, "最小电压(V)": false, 
            "平均视在功率(kVA)": false, "最大视在功率(kVA)": false, "最小视在功率(kVA)": false, "平均有功功率(kW)": false, "最大有功功率(kW)": false, "最小有功功率(kW)": false, };
      }
        break;
...
const handleClick = async (row) => {
   if(row.type != null  && row.type == 4){
    ...
    // console.log(queryParams.ipAddr,"ipaddr");
...
const handleExport1 = async () => {
  try {
    ...
    await download.excel(data, 'pdu历史数据详情.xlsx')
  } catch (error) {
    // 处理异常
